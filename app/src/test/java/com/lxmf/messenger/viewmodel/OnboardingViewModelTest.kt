...
class OnboardingViewModelTest {
    ...
    @Test
    fun `initial state has AUTO interface selected by default`() = runTest {
        val viewModel = createViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `initial state has empty display name`() = runTest {
        val viewModel = createViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `checks onboarding status on init`() = runTest {
    ...
    @Test
    fun `updateDisplayName clears error`() = runTest {
        val viewModel = createViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `toggleInterface adds interface when not selected`() = runTest {
        val viewModel = createViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `toggleInterface removes interface when already selected`() = runTest {
        val viewModel = createViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `toggleInterface can select multiple interfaces`() = runTest {
        val viewModel = createViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `onNotificationPermissionResult updates state when granted`() = runTest {
        val viewModel = createViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `onNotificationPermissionResult updates state when denied`() = runTest {
        val viewModel = createViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `onBlePermissionsResult updates state when granted`() = runTest {
        val viewModel = createViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `onBlePermissionsResult removes BLE from selection when denied`() = runTest {
        val viewModel = createViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `completeOnboarding saves display name`() = runTest {
        val viewModel = createViewModel()
    ...
    @Test
    fun `completeOnboarding uses default name when empty`() = runTest {
        val viewModel = createViewModel()
        val testIdentity = createTestIdentity()
        coEvery { mockIdentityRepository.getActiveIdentitySync() } returns testIdentity
        coEvery { mockIdentityRepository.updateDisplayName(any(), any()) } returns Result.success(Unit)
    ...
    @Test
    fun `completeOnboarding marks onboarding completed`() = runTest {
        val viewModel = createViewModel()
    ...
    @Test
    fun `completeOnboarding applies interface changes`() = runTest {
        val viewModel = createViewModel()
        coEvery { mockIdentityRepository.getActiveIdentitySync() } returns null
        advanceUntilIdle()
    ...
    @Test
    fun `completeOnboarding updates hasCompletedOnboarding state`() = runTest {
        val viewModel = createViewModel()
        coEvery { mockIdentityRepository.getActiveIdentitySync() } returns null
        advanceUntilIdle()
    ...
    @Test
    fun `completeOnboarding enables existing interface when selected`() = runTest {
    ...
    @Test
    fun `completeOnboarding disables existing interface when not selected`() = runTest {
    ...
    @Test
    fun `completeOnboarding creates interface when missing and selected`() = runTest {
    ...
    @Test
    fun `completeOnboarding creates TCP interface with default server`() = runTest {
        // Given: No interfaces exist
        every { mockInterfaceRepository.allInterfaceEntities } returns MutableStateFlow(emptyList())
        coEvery { mockIdentityRepository.getActiveIdentitySync() } returns null
        coEvery { mockInterfaceRepository.insertInterface(any()) } returns 1L
        ...
        coVerify {
            mockInterfaceRepository.insertInterface(match {
    ...
    @Test
    fun `completeOnboarding does not auto-create RNode interface`() = runTest {
        // Given: No interfaces exist
        every { mockInterfaceRepository.allInterfaceEntities } returns MutableStateFlow(emptyList())
        coEvery { mockIdentityRepository.getActiveIdentitySync() } returns null
        coEvery { mockInterfaceRepository.insertInterface(any()) } returns 1L
    ...
    @Test
    fun `completeOnboarding handles all selected interfaces correctly`() = runTest {
    ...
    @Test
    fun `skipOnboarding sets default display name`() = runTest {
        val viewModel = createViewModel()
        val testIdentity = createTestIdentity()
        coEvery { mockIdentityRepository.getActiveIdentitySync() } returns testIdentity
        coEvery { mockIdentityRepository.updateDisplayName(any(), any()) } returns Result.success(Unit)
    ...
    @Test
    fun `skipOnboarding creates only AutoInterface`() = runTest {
        val viewModel = createViewModel()
        coEvery { mockIdentityRepository.getActiveIdentitySync() } returns null
        every { mockInterfaceRepository.allInterfaces } returns MutableStateFlow(emptyList())
        coEvery { mockInterfaceRepository.insertInterface(any()) } returns 1L
    ...
    @Test
    fun `skipOnboarding does not create duplicate AutoInterface`() = runTest {
        val viewModel = createViewModel()
        coEvery { mockIdentityRepository.getActiveIdentitySync() } returns null
    ...
    @Test
    fun `skipOnboarding marks onboarding completed`() = runTest {
        val viewModel = createViewModel()
        coEvery { mockIdentityRepository.getActiveIdentitySync() } returns null
        every { mockInterfaceRepository.allInterfaces } returns MutableStateFlow(emptyList())
        advanceUntilIdle()
    ...
    @Test
    fun `skipOnboarding updates hasCompletedOnboarding state`() = runTest {
        val viewModel = createViewModel()
        coEvery { mockIdentityRepository.getActiveIdentitySync() } returns null
        every { mockInterfaceRepository.allInterfaces } returns MutableStateFlow(emptyList())
        advanceUntilIdle()
    ...
    @Test
    fun `completeOnboarding sets error state on exception`() = runTest {
        val viewModel = createViewModel()
    ...
    @Test
    fun `skipOnboarding sets error state on exception`() = runTest {
        val viewModel = createViewModel()
    ...
    @Test
    fun `completeOnboarding sets isSaving to true during operation`() = runTest {
        val viewModel = createViewModel()
        coEvery { mockIdentityRepository.getActiveIdentitySync() } returns null
        advanceUntilIdle()
